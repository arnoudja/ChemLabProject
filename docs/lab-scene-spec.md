...
Mixing parcels that change HCl concentration applies tabulated **integral dilution enthalpy** (exothermic when concentrating → diluting) as `ΔT = −Q / C_eff` after the sensible blend. Salt dissolve into acidic water reuses the water-solvent path; `enforce_saturation` preserves `h+` and includes its Cl⁻ in the mixed SI / unsaturated capacity.

Inspect shows **pH** for aqueous acid: strong-acid approx `pH = −log₁₀([H⁺])` with `[H⁺] = n_h+ / V_solution` (litres).
