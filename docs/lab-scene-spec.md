...
While the **burner** is on and heating the dish, dish temperature rises with `dT = (BURNER_POWER_W / C_eff) · dt` toward the composition-aware boil point `T_boil(x_w)` (Antoine vapor pressure + Raoult: `x_w · P_sat(T) = P_atm`). Fixed lab air: **RH = 50%**, **P_atm = 1.00 bar**, ambient **20 °C**. Water mole fraction `x_w` counts liquid water and aqueous ions only (sand / undissolved solids excluded).

**Burner off (sub-boil):** mass-transfer loss `m_dot = k · A · max(0, p_w − p_air) / P_atm` with `p_w = x_w · P_sat(T)`, `p_air = RH · P_sat(T_amb)`; `A = DISH_EVAP_AREA_M2`, `k` chosen so pure water at 20 °C / 50% RH loses ≈ **2 ml/h**. Every water mass loss cools the dish by `ΔT = −m · H_vap / C_eff` (`WATER_LATENT_HEAT_J_PER_G = 2257`). Sub-boil mass transfer is skipped while the burner is heating so Antoine-scaled driving force cannot dry/concentrate the dish before boil.

**At boil** (burner on, `p_w ≥ P_atm`): clamp `T` to `T_boil`; heat-limited rate `m_dot = BURNER_POWER_W / H_vap` (~0.487 ml/s at `BURNER_POWER_W = 1100`). While heating, Newton cool is skipped for the dish (same as before), so boil `Q_net` is burner power only. Burner off at/above boil does not heat-limit evaporate (`Q_net ≤ 0`); ambient cool lowers `T`. Beakers never evaporate. Ions stay behind; mixed NaCl/CaCl₂ equilibrium then precipitates or redissolves on **every aqueous vessel**. The burner turns off when the dish has no liquid.
